...
namespace ServiceFabric.WindowsPatchInstaller
{
    ...
    [EventSource(Name = "ServiceFabricSamples-PathInstallerApp-PatchInstallerService")]
    internal sealed class ServiceEventSource : EventSource
    {
        ...
        public static class Keywords
        {
            ...
            public const EventKeywords PatchInstallation = (EventKeywords)0x3L;
        }
        #endregion
        ...
        private const int PatchDataPackageNotFoundEventId = 7;
        [Event(
            PatchDataPackageNotFoundEventId,
            Level = EventLevel.Warning,
            Message = "Failed to get the data package for patch '{0}'",
            Keywords = Keywords.PatchInstallation)]
        public void PatchDataPackageNotFound(string patchId, string exception = "")
        {
            this.WriteEvent(PatchDataPackageNotFoundEventId, patchId, exception);
        }

        private const int PatchInstallFileNotFoundEventId = 8;
        [Event(
            PatchInstallFileNotFoundEventId,
            Level = EventLevel.Warning,
            Message = "The install file '{0}' for patch '{1}' is not present in the data package '{1}'.",
            Keywords = Keywords.PatchInstallation)]
        public void PatchInstallFileNotFound(string fileName, string patchId)
        {
            this.WriteEvent(PatchInstallFileNotFoundEventId, fileName, patchId);
        }

        private const int PatchProcessCompletedEventId = 9;
        [Event(
            PatchProcessCompletedEventId,
            Level = EventLevel.Informational,
            Message = "The file '{0}' for patch '{1}' exited with code '{2}'.",
            Keywords = Keywords.PatchInstallation)]
        public void PatchProcessCompleted(string fileName, string patchId, int exitCode)
        {
            this.WriteEvent(PatchProcessCompletedEventId, fileName, patchId, exitCode);
        }

        private const int FailedToStartPatchProcessEventId = 10;
        [Event(
            FailedToStartPatchProcessEventId,
            Level = EventLevel.Informational,
            Message = "Failed to start the process '{0}' for installing/verifying patch '{1}'.",
            Keywords = Keywords.PatchInstallation)]
        public void FailedToStartPatchProcess(string fileName, string patchId, string exception = "")
        {
            this.WriteEvent(FailedToStartPatchProcessEventId, fileName, patchId, exception);
        }
